...
Initializing analysis system\\.\\.\\."""
    ...
    async def agent_started(self, agent_name: str, description: str) -> None:
        ...
        self.agent_history.append(
            {'name': agent_name, 'start_time': self.agent_start_time, 'status': 'running'}
        )
...
Status: ⚙️ Processing\\.\\.\\."""
    ...
    async def analysis_complete_detailed(self) -> None:
        ...
        for agent in self.agent_history:
            ...
            plots_info = (
                f" \\({len(agent.get('plots_created', []))} plots\\)"
                if agent.get('plots_created')
                else ""
            )
            tools_info = (
                f" \\({agent.get('tool_calls', 0)} tools\\)"
                if agent.get('tool_calls', 0) > 0
                else ""
            )
            cost_info = (
                f" \\({self._format_cost(agent.get('cost_usd', 0))[:-1]}\\)"
                if agent.get('cost_usd', 0) > 0
                else ""
            )

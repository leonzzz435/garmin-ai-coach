...
from core.security import SecureCompetitionManager, SecureCredentialManager, StorageError
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("help_") or query.data.startswith("roadmap_"):
        ...
        if category in messages:
            await query.message.reply_text(messages[category], parse_mode=ParseMode.MARKDOWN_V2)
            return
    ...
    if handler:
        try:
            await query.message.reply_text(
                escape_markdown("🔄 Processing your request..."), parse_mode=ParseMode.MARKDOWN_V2
            )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.effective_user:
        ...
        await message.reply_text(
            "❌ An error occurred. Unable to identify user.", parse_mode=ParseMode.MARKDOWN_V2
        )
        return

    # Track user interaction
    user = update.effective_user
    user_tracker.track_user(user_id=user.id, first_name=user.first_name, username=user.username)
    ...
    await message.reply_text(
        escape_markdown("AI Training Assistant")
...
async def delrace(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not race:
            ...
            if upcoming:
                response = (
                    escape_markdown("❌ No race found on that date.\n\n") + "*Available Races:*\n"
                )
                for race in upcoming:
                    date_str = race.date.strftime("%Y-%m-%d")
                    response += (
        ...
        if deletion_success:
            ...
            await message.reply_text(success_message, parse_mode=ParseMode.MARKDOWN_V2)
            ...
            await message.reply_text(
                "❌ Failed to delete race\\. Please try again\\.", parse_mode=ParseMode.MARKDOWN_V2
            )

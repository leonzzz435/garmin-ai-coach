...
async def plot_resolution_node(state: TrainingAnalysisState) -> TrainingAnalysisState:
    ...
    try:
        ...
        logger.info(
            f"Found {len(plots_data)} plot entries and {len(plot_storage_data)} plot storage entries"
        )
        ...
        resolved_count = (
            validation_result['total_references'] - final_validation['total_references']
        )
        ...
        logger.info(
            f"Plot resolution completed: {resolved_count}/{validation_result['total_references']} plots resolved"
        )

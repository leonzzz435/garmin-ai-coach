...
class LangChainWeeklyPlanOrchestrator:
    ...
    async def run_weekly_planning(
        self, store_intermediate: bool = True
        ...
        try:
            ...
            if self.progress_manager:
                await self.progress_manager.agent_started(
                    "Plan Formatter Agent", "Formatting training plan into structured HTML report"
                )
            ...
            with get_usage_metadata_callback() as cb:
                ...
                async def call_formatter():
                    return await formatter_chain.ainvoke(
                        {'season_plan': season_plan, 'weekly_plan': weekly_plan}
                    )
            ...
            if store_intermediate:
                try:
                    ...
                    logger.info(
                        f"Stored {len(stored_files)} intermediate planning files for {self.athlete_name}"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to store intermediate planning results for {self.athlete_name}: {e}"
                    )

            logger.info(
                f"Weekly planning orchestration completed successfully for user {self.user_id}. "
...
class LangChainWeeklyPlanFlow:
    ...
    async def run_weekly_planning(
        ...
        Returns:
            Tuple of (HTML formatted weekly plan, dict of intermediate results)
        """
        orchestrator = LangChainWeeklyPlanOrchestrator(

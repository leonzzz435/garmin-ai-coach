...
from services.ai.tools.plotting import PlotStorage, PlotReferenceResolver
...
class LangChainAnalysisOrchestrator:
    ...
    def __init__(self, garmin_data: GarminData, user_id: str, athlete_name: str,
                 analysis_context: str = "", max_plots_per_agent: int = 2,
                 tool_limiter=None, progress_manager: Optional[Any] = None):
    ...
    async def run_analysis(self, store_intermediate: bool = True) -> tuple[str, dict[str, str]]:
        ...
        try:
            ...
            # Get tool usage
            tool_usage = self.chains.get_tool_usage_stats().get('metrics_agent', {}).get('total_calls', 0)
            ...
            # Get tool usage
            tool_usage = self.chains.get_tool_usage_stats().get('activity_interpreter_agent', {}).get('total_calls', 0)
            ...
            # Get tool usage
            tool_usage = self.chains.get_tool_usage_stats().get('physiology_agent', {}).get('total_calls', 0)
            ...
            # Get tool usage
            tool_usage = self.chains.get_tool_usage_stats().get('synthesis_agent', {}).get('total_calls', 0)
...
class LangChainAnalysisFlow:
    ...
    @staticmethod
    async def run_analysis(garmin_data: GarminData, user_id: str, athlete_name: str,
                          analysis_context: str = "", max_plots_per_agent: int = 2,
                          tool_limiter=None, progress_manager: Optional[Any] = None) -> tuple[str, dict[str, str]]:

...
from services.ai.utils.retry_handler import retry_with_backoff, AI_ANALYSIS_CONFIG, get_error_details
    ...
    async def run_analysis(self, store_intermediate: bool = True) -> tuple[str, dict[str, str]]:
        ...
        try:
            ...
            with get_usage_metadata_callback() as cb:
                # Use retry mechanism for AI analysis calls
            ...
            with get_usage_metadata_callback() as cb:
                # Use retry mechanism for AI analysis calls
                async def call_activity_interpreter():
                    return await activity_interpreter_chain.ainvoke(
                        {
                ...
                activity_response = await retry_with_backoff(
                    call_activity_interpreter,
                    AI_ANALYSIS_CONFIG,
                    "Activity Interpreter Agent"
                )
            ...
            with get_usage_metadata_callback() as cb:
                # Use retry mechanism for AI analysis calls
                async def call_synthesis_agent():
                ...
                synthesis_response = await retry_with_backoff(
                    call_synthesis_agent,
                    AI_ANALYSIS_CONFIG,
                    "Synthesis Agent"
                )
            ...
            with get_usage_metadata_callback() as cb:
                # Use retry mechanism for AI analysis calls
                async def call_formatter_agent():
                    return await formatter_chain.ainvoke({'synthesis_result': synthesis_result})
                
                html_result = await retry_with_backoff(
                    call_formatter_agent,
                    AI_ANALYSIS_CONFIG,
                    "HTML Formatter Agent"
                )
            ...
            intermediate_results = {
                'metrics_result': metrics_result,
                'activity_summary': self._extract_agent_output(activity_summary),
                'activity_result': activity_result,
                'physiology_result': physiology_result,

...
class MasterOrchestrator:
    ...
    async def run_full_analysis(
        ...
        try:
            ...
            planning_html, planning_intermediates = (
                await LangChainWeeklyPlanFlow.run_weekly_planning(
            ...
            # Provide detailed error information for different types of failures
            error_details = get_error_details(e)
            
            if isinstance(e, anthropic.APIStatusError):
                if is_anthropic_overload_error(e):
                    logger.error(
                        f"Full analysis failed for user {user_id} due to API overload. "
                        f"The system automatically retried but API remains overloaded: {error_details}"
                    )
                else:
                    logger.error(
                        f"Full analysis failed for user {user_id} due to API error: {error_details}"
                    )
            else:
                logger.error(f"Full analysis failed for user {user_id}: {error_details}")
            
            # Re-raise the exception for handling by upstream components
            raise
...
class LangChainFullAnalysisFlow:
    ...
    async def run_full_analysis(
        ...
        Returns:
            Dict containing both analysis and planning results
        """
        return await MasterOrchestrator.run_full_analysis(
            user_id, athlete_name, garmin_data, analysis_context, planning_context, progress_manager
        )

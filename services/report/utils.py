    ...
    try:
        ...
        for idx, activity in enumerate(activities):
            if 'error' in activity:
                ...
                continue
            logger.info(
                f"Activity {idx} type: {activity['activity_type']}, name: {activity['activity_name']}"
            )
        ...
        total_stats = {
            ...
            'total_distance': sum(
                (a['summary']['distance'] or 0) if a['summary'] else 0 for a in activities
            )
            / 1000,  # km
            'total_duration': sum(
                (a['summary']['duration'] or 0) if a['summary'] else 0 for a in activities
            )
            / 3600,  # hours
            'total_calories': sum(
                (a['summary']['calories'] or 0) if a['summary'] else 0 for a in activities
            ),
        }
...
def summarize_training_volume(activities: list[dict[str, Any]]) -> str:
    try:
        ...
        weekly_summary = (
            df.groupby('week')
            .agg({'duration': 'sum', 'distance': 'sum', 'startTime': 'count'})
...
def summarize_training_intensity(activities: list[dict[str, Any]]) -> str:
    try:
        ...
        for activity in activities:
            try:
                if activity.get('activity_type') == "multisport" and activity.get(
                    'childActivities'
                ):
                    for child in activity['childActivities']:
                        try:
                            intensity_data.extend(
                                _extract_intensity_data(
                                    child, activity.get('start_time', 'Unknown')
                                )
                            )
                        except Exception as e:
                    ...
                    intensity_data.extend(
                        _extract_intensity_data(activity, activity.get('start_time', 'Unknown'))
                    )
            except Exception as e:
...
def _extract_intensity_data(activity: dict[str, Any], date: str) -> list[dict[str, Any]]:
    try:
        if not activity.get('hr_zones'):
            logger.warning(
                f"No HR zones found for activity type: {activity.get('activity_type', 'Unknown')}"
            )
            return []
...
def summarize_recovery(recovery_data: list[dict[str, Any]]) -> str:
    try:
        ...
        for _, row in df.sort_values('date', ascending=False).iterrows():
            ...
            sleep_duration = (
                f"{row['sleep_duration']:.1f}" if pd.notnull(row['sleep_duration']) else 'N/A'
            )
            ...
            stress_level = (
                f"{row['stress_level']:.0f}" if pd.notnull(row['stress_level']) else 'N/A'
            )
...
def summarize_training_load(load_history: list[dict[str, Any]]) -> str:
    try:
        ...
        weekly_load = (
            df.groupby('week')
            .agg({'acute_load': 'mean', 'chronic_load': 'mean', 'acwr': 'mean'})
            .round(2)
        )
...
def summarize_vo2max_evolution(vo2max_history: list[dict[str, Any]]) -> str:
    try:
        ...
        for item in vo2max_history:
            try:
                processed_data.append({'date': item.get('date'), 'value': item.get('value', 0)})
            except Exception as e:
...
def summarize_endurance_score_weekly(endurance_scores: list[dict[str, Any]]) -> str:
    try:
        ...
        weekly_scores = df.groupby('week').agg({'overall_score': 'mean', 'classification': 'last'})

...
def test_state_schema_completeness():
    ...
    required_fields = [
        'user_id', 'athlete_name', 'garmin_data', 'execution_id',
        'metrics_result', 'activity_summary', 'activity_result',
        'physiology_result', 'synthesis_result', 'analysis_html',
        'plots', 'costs', 'errors'
    ]
...
async def test_node_basic_functionality(mock_get_llm, basic_test_state):
    ...
    assert 'costs' in result or 'errors' in result  # Either success with costs or failure with errors
...
def test_workflow_structure_stability():
    try:
        with patch('services.ai.langgraph.config.langsmith_config.LangSmithConfig.setup_langsmith'):
            from services.ai.langgraph.workflows.analysis_workflow import create_analysis_workflow, create_simple_sequential_workflow

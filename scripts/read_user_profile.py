...
def read_user_profile(user_id: str) -> None:
    try:
        ...
        logger.info(
            f"Lactate Threshold Speed: {user_profile.get('lactate_threshold_speed', 'Not set')} m/s"
        )
        logger.info(
            f"Lactate Threshold HR: {user_profile.get('lactate_threshold_heart_rate', 'Not set')} bpm"
        )
        logger.info(f"FTP Auto Detected: {user_profile.get('ftp_auto_detected', 'Not set')}")
        logger.info("\n=== Training Schedule ===")
        logger.info(
            f"Available Training Days: {', '.join(user_profile.get('available_training_days', ['Not set']))}"
        )
        logger.info(
            f"Preferred Long Training Days: {', '.join(user_profile.get('preferred_long_training_days', ['Not set']))}"
        )
